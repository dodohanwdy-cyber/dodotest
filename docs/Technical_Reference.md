# 열고닫기(OPCL) 기술 레퍼런스 (Technical Reference) - 2026.05.08 최신화
...
### 3. 보안 접근 및 세션 관리 (Supabase 완전 이관)
- **데이터베이스 및 인증 시스템**: 모든 사용자 정보와 DB 운영이 **Supabase**로 완전 이관되었습니다. Supabase Auth를 통해 이메일/비밀번호 인증을 처리하며, 세션은 Supabase Client SDK로 전역 상태(`AuthContext`)로 관리됩니다.
- **n8n 웹훅 보안 통신 (API 클라이언트)**:
  - `src/lib/n8nClient.ts`의 전용 HTTP 인터셉터를 사용하여 n8n 서버와 통신합니다.
  - 도메인 화이트리스트(`https://primary-production-1f39e.up.railway.app/`) 검증을 거친 후, `X-Api-Key` 헤더에 환경 변수(`NEXT_PUBLIC_N8N_API_KEY`)를 자동으로 주입하여 통신 보안을 강화했습니다.
  - 비밀번호 및 중요 키는 하드코딩되지 않고 모두 `.env.local`로 분리되어 관리됩니다.
- **내담자 리포트 (`/report/[id]`)**: 
