# 📑 제품 요구사항 정의서 (PRD) 및 개발 인수인계서

> **문서 목적:** 프로젝트의 핵심 요구사항, 시스템 아키텍처 및 모든 워크플로우 명세를 정리하여 원활한 유지보수와 인수인계를 지원하는 데 목적이 있습니다.

**프로젝트 명:** 도도한콜라보 (OPCL) AI 상담 및 매니지먼트 플랫폼
**작성일:** 2026-04-21
**대상 독자:** 프론트엔드/백엔드 유지보수 및 신규 개발을 담당할 중/고급 프로그래머
**주요 아키텍처:** Next.js (App Router) 기반 서버리스 프론트엔드 + n8n 워크플로우 전면 백엔드(BaaS)

---

## 🧭 1. 서비스 목적 및 핵심 유저 시나리오 (Service Objective & User Flow)

**열고닫기(OPCL) 서비스의 핵심 목적**은 청년들이 심리적 문턱을 낮추고 상담을 통해 필요한 맞춤형 정부/지원 정책에 더 쉽게 도달할 수 있도록 돕는 데 있습니다. 본 시스템은 이 과정을 AI와 디지털화로 효율화하여 내담자에게 최적화된 리포트를 제공하고, 상담사(매니저)의 데이터 정리 및 일정 관리 업무를 극적으로 단축시킵니다.

### 핵심 유저 활동 (User Activities)
1. **내담자 (Client)의 인테이크(Intake) 여정**:
   - 회원가입 후, 내 정보와 고민 카테고리를 입력합니다 (Step 1).
   - 희망 상담 일정을 매니저 캘린더에서 선택하여 예약(pending) 합니다 (Step 2).
   - AI와 심층 대면 전 사전 채팅을 진행하며 위험도 및 키워드 데이터를 백그라운드로 전송합니다 (Step 3).
   - 이후 매니저가 일정을 확정하면 대시보드에서 **프리미엄 확정 배너**를 확인하고, 상담 완료 후 **보안 리포트**를 자체 열람합니다.

2. **상담사/매니저 (Manager)의 관리 여정**:
   - 매니저 대시보드에서 들어온 인테이크 건을 확인하고, 캘린더에 드래그 앤 드롭하여 일정을 확정(confirmed) 지으며 알람을 보냅니다.
   - 실제 상담 시 실시간 AI 분석 가이드를 활용해 상담의 질을 높입니다.
   - 상담 종료 후 클릭 한 번으로 AI 리포트를 즉시 생성·발견하여 내담자에게 공유합니다.

---

## 🏗 2. 시스템 아키텍처 (Architecture Overview)

본 서비스는 전통적인 RDBMS/Node.js 백엔드를 직접 구축하지 않고, **Supabase**와 **n8n 워크플로우**를 결합하여 개발 효율성과 보안성을 극대화한 구조를 가집니다.

- **프론트엔드 (Next.js)**: UI 상태 관리, 사용자 입력 검증, 화면 전환 및 백엔드 연동(Supabase/n8n) 담당.
- **인증 및 계정 관리 (Supabase Auth)**: 비밀번호 암호화, 로그인/회원가입 세션, 역할(Role) 기반 접근 제어(RBAC) 등 인증의 핵심 기능을 담당합니다.
- **프로필 데이터베이스 (Supabase Database)**: `user_profiles` 테이블을 통해 사용자의 역할 및 상세 프로필 데이터를 관리합니다.
- **비즈니스 백엔드 (n8n)**: 상담 데이터 저장, AI 분석, 알림 발송 등 복잡한 비즈니스 로직을 Webhook 기반으로 처리합니다.
- **설정 파일 (`src/config/webhooks.js`)**: 모든 백엔드 엔드포인트 URL 상수를 중앙에서 매핑 관리합니다.

---

## 📂 3. 프론트엔드 라우팅 및 기능 명세

디렉토리 구조는 크게 인증(`Auth`), 내담자(`Client`), 매니저(`Manager`)로 분리됩니다.

### 3.1. 인증 및 세션 영역 (Auth)
- `/login` / `/signup`: **Supabase Auth SDK** 기반의 계정 생성 및 인증 처리.
- `AuthContext` 상태 관리: `supabase.auth.onAuthStateChange` 및 `user_profiles` 테이블 연동을 통해 전역 유저 세션 및 매니저 권한(`userRole`)을 실시간으로 관리합니다.

### 3.2. 내담자 영역 (Client)
- `/client/dashboard`
  - **역할**: 현재 상담 진행 상태(status) 확인 및 확정 일정 확인.
  - **주요 기능**: 확정(`confirmed`) 상태 시 '일정 확정 배너' 노출. 완료 후 리포트 열람.
- `/client/intake`
  - **역할**: 3단계 신청 동적 폼. 메인 화면에서 `?mode=new` 로 진입 시 로컬 스토리지에 남아있던 모든 이전 진행 중인 세션을 초기화하여 새 신청이 꼬이지 않도록 보호합니다.
- `/report/[id]` (내담자 리포트 공용 페이지)
  - **역할**: 매니저가 발급한 AI 리포트를 내담자가 열람.
  - **강조 사항**: 긴 텍스트를 피하기 위해 모바일 친화적인 **슬라이드 탭 뷰(진단 / 정책 / 향후 계획)** 와 **채팅 말풍선 UI**, **인터랙티브 투두 리스트(To-Do)** 형식으로 최적화되어 있습니다.

### 3.3. 매니저(상담가) 영역 (Manager)
- `/manager/dashboard`
  - **역할**: 전체 내담자 접수 현황 조회, 달력 기반 드래그 앤 드롭 배정 조율.
  - **핵심 변동성**: `ScheduleAdjustPopup` 호출 시, 여러 웹훅에서 요청자 구분을 위해 접속한 매니저의 로그인된 상태 객체에서 추출한 유저 이메일을 동적으로 할당하여 전달합니다. 배정이 끝나면 프론트 상태(`data`)를 즉시 업데이트하여 웹훅 딜레이 없이 시각적 피드백을 제공합니다.
- `/manager/consultation/[id]/report`
  - **역할**: 실무 상담 화면 및 결과 작성. AI가 답변한 `1. 2.` 포맷 등 줄글 형태의 데이터를 프론트(Smart List Component)에서 감지하여 다이내믹한 카드 리스트 뷰로 파싱해줍니다.
- `/manager/completed`
  - **추가 기능**: 최근 월 기준 필터링을 기본으로 제공하며, **"전체 내역 보기"** 버튼에 토글 기능을 달아 빠르게 필터 해제를 제어할 수 있습니다.

---

## 🔗 4. n8n 워크플로우 연동 명세 (18개 핵심)

서버(n8n) 내에서 실행 중인 비즈니스 로직 워크플로우 명세입니다. 인증 기능이 Supabase로 이관됨에 따라 실질적인 비즈니스 데이터 처리에 집중하도록 개편되었습니다.

| n8n 워크플로우 명 (n8n에서 검색 시) | 적용 페이지 / 컴포넌트 | 기능 설명 및 역할 |
|---|---|---|
| `[Client-Dash] 01. 대시보드 신청 현황 조회 (/dashboard-applications)` | `/client/dashboard` | 해당 유저의 최근 신청/진행 중인 상태 정보 로드 |
| `[Client-Dash] 02. 전체 신청 데이터 상세조회 (/application-detail)` | `/client/dashboard` | 튜토리얼 또는 과거 결과물 상세 열람 |
| `[Client-Intake] S1. 기본정보 및 정책 폼 제출 (/submit-intake)` | `/client/intake` (S1 폼) | S1 통과 시 저장, 다음 S2 스텝 개방 응답 반환 |
| `[Client-Intake] S2. 캘린더 상담 가능 일정 조회 (/time-check)` | `/client/intake` (S2 캘린더) | 실시간 매니저/회사 캘린더 DB 조회하여 남은 시간 리턴 |
| `[Client-Intake] S2. 상담 선호 일정 신청 (/time-request)` | `/client/intake` (S2 제출) | 선택한 일정을 예약 완료(`pending`/`reserved`) 상태로 변경 |
| `[Client-Intake] S3. AI 사전 인터뷰 데이터 분석 (/request-data)`| `/client/intake` (S3 채팅) | 채팅 로그 전체 데이터베이스 스태킹 및 요약 응답 |
| `[Mgr-Dash] 01. 전체 상담 신청 현황 조회 (/schedule-start)` | `/manager/dashboard` 메인 | 접수된 유저 목록과 달력 렌더링을 위한 전역 데이터 로드 |
| `[Mgr-Dash] 02. 스케줄 확정 및 알람 발송 (/send-confirmed-list-data)`| `/manager/dashboard` 배너쪽 | 스케줄을 확정 짓고 알람 트리거 (Client 배너 작동용) |
| `[Mgr-Dash] 03. 드래그앤드롭 일정 재조정 (/schedule-confirm)` | `/manager/dashboard` 드래그 | 캘린더 달력 블록 이동 시 DB 시간(타임스탬프) 즉각 Update |
| `[Mgr-Dash] 04. 스케줄 취소 처리 (/cancel-assignment)` | 대시보드 취소 버튼 | 예약 취소 시 가용 시간 원복 및 DB 변경 |
| `[Mgr-Dash] 05. 상담 전 맞춤 정책 탐색 (/policy-search)` | 매니저 툴킷 | 내담자의 조건에 맞춘 청년 정책 DB 서치 보조 |
| `[Mgr-Dash] 06. 확정 일정 초기화 처리 (/reset-schedule)` | 매니저 리셋 버튼 | 강제 일정 롤백용 |
| `[Mgr-Dash] 07. 확정 상담 내용 일괄 로드 (/send-all)` | 전역 상태 데이터 로드 | |
| `[Mgr-Dash] 08. 개별 상담 내역 즉시 로드 (/check-case)` | 상담 내역 모달 | 특정 `requestId`에 대한 요약/채팅 로그 상세 로드 |
| `[Mgr-Consult] 01. 상담 시작 전 통합 데이터 로딩 (/send-all-data)` | `/manager/consultation/[id]` | 실무 상담 진입 시 페이지 초기화 용도 |
| `[Mgr-Consult] 02. 최종 상담 요약 및 리포트 제출 (/consultation-summary)` | 실시간 AI 정리 | 상담 중 넘어가는 STT 텍스트 구조화 및 리포트 초안 발행 |
| `[Mgr-Completed] 01. 완료된 전체 상담 목록 조회 (/completed-list)` | `/manager/completed` 메인 | 완료(`completed`) 상태인 신청건 히스토리 로딩 |
| `[Mgr-Completed] 02. 완료 상담 최종 분석결과 조회 (/get-completed-detail)`| 리포트 세부 모달 | 생성된 리포트 원문 및 엑세스 링크 획득 |

---

## 🚨 5. 에러 감시 및 로깅 파이프라인 (Ops)

이번 업데이트를 통해 **n8n 백엔드 장애(Timeout, DB 락, AI 토큰 오류 등)를 개발자가 슬랙에서 즉각 모니터링할 수 있는 완전 자동화 시스템**을 부착했습니다.

1. **마스터 알림 봇**: n8n 목록 검색 시 맨 최상단(`00. 시스템 오류 마스터 알림 봇`)에 위치합니다. 
2. 이 워크플로우 안에는 `Error Trigger` 설정이 들어 있으며, 사내 지정된 Slack 웹훅으로 다음과 같은 데이터를 POST 합니다:
   - 에러가 발생한 워크플로우 이름 / 멈춘 노드 이름 / 에러 상세 메세지 / 발생 타임스탬프
3. **작동 원리**: 위 `3번 항목`에 기재된 21개의 운영 워크플로우들(총 27개 적용됨)의 Workflow Settings를 확인해 보면 `Error Workflow` 탭에 `00. 시스템 오류 마스터 알림 봇`의 ID(`U4BH9faxwXYzq8BA`)가 하드코딩 되어 있습니다.
4. **유지보수 지침**: 차후 새로운 워크플로우(기능)를 추가할 때는 반드시 해당 워크플로우 세팅(`Settings > Error Workflow`)에 마스터 봇을 지정해주어야 중앙 슬랙 관제가 이루어집니다.

---
**※ 해당 PRD는 2026.04.21 기준으로 최신화되었으며, 지속적인 보안 강화 및 아키텍처 개선 작업 진행 상태에 맞춰 업데이트가 필요합니다.**

## 🚀 5. 향후 엔지니어링 로드맵 (Roadmap)

시스템의 안정성과 확장성을 위해 아래와 같은 기술적 개선이 예정되어 있습니다. 유지보수 시 참고하시기 바랍니다.

1.  **데이터베이스 이관**: 현재 n8n과 구글 시트로 연동된 비즈니스 데이터 처리 로직을 점진적으로 **Supabase Database**로 통합할 예정입니다.
2.  **API 표준화**: n8n 플랫폼 의존도를 낮추고 Next.js 서버리스 API를 활용한 데이터 처리를 강화할 계획입니다.
3.  **보안 강화**: RBAC 시스템을 더욱 세분화하여 각 상담 건별 접근 제어를 데이터베이스 레벨(RLS)에서 구현할 예정입니다.
